(function (factory) {
    ...
    function ($) {
        ...
        let stylFix = $(document.createElement("style"))
            .appendTo(document.head)
            .addClass("ripple-custom-config");
        let stylSet = $(document.createElement("style"))
            .appendTo(document.head)
            .addClass("ripple-coordinates");
        let stylObj = {};
        let stylPos = {};
        let nowRuns = 0;
        ...
        $.fn.ripple = function (opt) {
            ...
            $(this).each((i, v) => {
                ...
                if (typeof opt === "string") {
                    switch (opt) {
                        ...
                        case "onclick":
                            ...
                            break;
                        case "enable":
                            $elm.removeClass("ripple-disabled").bind(
                                $elm[0].rippleVars.ondown
                                    ? pointerdown
                                    : "click",
                                $elm[0].rippleVars.callback
                            );
                        ...
                        case "disable":
                            ...
                            break;
                        case "ripple":
                            $elm.removeClass("ripple-disabled").trigger(
                                $elm[0].rippleVars.ondown
                                    ? "mousedown"
                                    : "click"
                            );
                ...
                def.duration =
                    typeof def.duration == "number"
                        ? def.duration
                        : /^([2-9]|10)x$/i.test(def.duration)
                        ? def.duration.toLowerCase()
                        : globDur;
                let rePos = (e) => {
                    // console.log(e);
                    ...
                    if (
                        (x1 == 0 && y1 == 0) ||
                        $elm[0].rippleVars.type == "trigger"
                    )
                        stylPos[$elm[0].rippleVars.rid] = ``;
                    // && e.pageX == 0 && e.pageY == 0 && e.clientX == 0 && e.clientY == 0
                    else {
                        let off = $elm.offset();
                        stylPos[$elm[0].rippleVars.rid] = `
                ...
                $elm[0].rippleVars = {
                    ...
                    ease: def.ease,
                    type: def.type,
                    time: /^([2-9]|10)x$/i.test(def.duration)
                        ? parseInt(def.duration) * globDur
                        : def.duration,
                    timeout: 0,
                    ondown: !def.onclick,
                    callback: (e) => {
                ...
                $elm
                    ...
                    .addClass(
                        "ripple-" +
                            (def.type == "trigger" ? "trigger" : "element") +
                            iconClass +
                            (def.disable ? " ripple-disabled" : "") +
                            " " +
                            $elm[0].rippleVars.idx
                    )
                    .addClass($elm[0].rippleVars.rid)
                    .addClass(
                        $elm[0].rippleVars.size >= 2 &&
                            $elm[0].rippleVars.size <= 10
                            ? "ripple-anim-" + $elm[0].rippleVars.size + "x"
                            : ""
                    )
                    .addClass(
                        /^([2-9]|10)x$/i.test(def.duration)
                            ? "ripple-time-" + parseInt(def.duration) + "x"
                            : ""
                    )
            ...
            if (typeof opt === "object") {
                ...
                if (
                    (def.duration !== globDur &&
                        /^([2-9]|10)x$/i.test(def.duration) === false) ||
                    def.ease !== globEse
                )
                    stylObj[selectorIDX] =
                        `.ripple-${
                            ...
                            def.type == "trigger" ? " .ripple-target" : ""
                        }::after {` +
                        // `${ def.size !== 1 ? 'animation-name: rippleanim-'+def.size+'x;' : '' }`+
                        `${
                            def.ease !== globEse
                                ? "animation-timing-function: " + def.ease + ";"
                                : ""
                        }` +
                        `${
                            def.duration !== globDur &&
                            /^([2-9]|10)x$/i.test(def.duration) === false
                                ? "animation-duration: " + def.duration + "ms;"
                                : ""
                        }` +
                        `}`;
                let stylTxt = "";
