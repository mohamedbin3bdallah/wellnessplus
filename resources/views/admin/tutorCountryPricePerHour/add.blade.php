...
@section('title', __('adminstaticword.add').' '.__('adminstaticword.Country').' '.__('adminstaticword.pricePerHour'))
...
<section class="content">
  <div class="row">
    ...
    <div class="box-header with-border">
      <div class="box-title">
		{{ __('adminstaticword.add').' '.__('adminstaticword.Country').' '.__('adminstaticword.pricePerHour') }}
      </div>
    </div>
    <form action="{{ route('store.tutor.country.pricePerHour') }}" method="POST">
    @csrf
      <div class="box-body">
		  <div class="form-group">
              ...
              <select required class="form-control js-example-basic-single" name="tutor_id">
                ...
                @foreach ($tutors as $tutor)
                    @if(isset($tutor->user))<option value="{{ $tutor->id }}" @if(old('tutor_id') == $tutor->id) {{ 'selected' }} @endif>{{ $tutor->user->fname.' '.$tutor->user->lname.' - '.$tutor->user->email }}</option>@endif
                @endforeach
			  </select>
		  ...
		  <div class="form-group">
              ...
              <select required class="form-control js-example-basic-single" name="country_id">
                ...
                @foreach ($countries as $country)
                    <option value="{{ $country->id }}" @if(old('country_id') == $country->id) {{ 'selected' }} @endif>{{ $country->nicename }}</option>
                @endforeach
			  </select>
		  ...
		  <div class="form-group">
              ...
              <input required type="text" class="form-control" name="pricePerHour" value="{{ old('pricePerHour') }}">
          ...
          <div class="form-group">
              ...
              <select required class="form-control" name="status">
                  <option value="0" @if(old('status') == 0) {{ 'selected' }} @endif>{{ __('adminstaticword.notActive') }}</option>
                  <option value="1" @if(old('status') == 1) {{ 'selected' }} @endif>{{ __('adminstaticword.Active') }}</option>
              </select>
