...
@section('title', __('adminstaticword.Appointment'))
@section('body')
        ...
        <form action="{{route('appointment.update',$appoint->id)}}" method="POST" enctype="multipart/form-data">
            ...
            <div class="row" style="{{ $appoint['accept'] == '1' ? '' : 'display:none' }}" id="sec1_one">
              <div class="col-md-12">
                ...
                <textarea id="reply" name="reply" rows="1" class="form-control" placeholder="{{ __('adminstaticword.Reply') }}">{{ $appoint['reply'] }}</textarea>
              </div>
            </div>
            <br>
