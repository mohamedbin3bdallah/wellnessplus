	  ...
	  <div class="col-xs-12">
	    <div class="box box-primary" >
	      	<div class="box-header with-border">
	        	<h3 class="box-title">{{ __('dashboard.cities') }}</h3>
		                ...
		                @foreach ($cities as $city)
			                ...
			                <tr>
			                  ...
			                  <td><form  method="post" action="{{url('admins/city/'.$city->id)}}" data-parsley-validate class="form-horizontal form-label-left">
			                      {{ csrf_field() }}
			                      {{ method_field('DELETE') }}
			                       <button  type="submit" class="btn btn-danger" onclick="return confirm('{{ __('dashboard.delete_this_city') }}')" ><i class="fa fa-fw fa-trash-o"></i></button></td>
			                      </form>
			                  </td>
			                </tr>
		                @endforeach
            ...
            <form id="demo-form2" method="post" action="{{route('city.manual')}}" data-parsley-validate class="form-horizontal form-label-left">
              {{ csrf_field() }}
				...
				<div class="row">
	                ...
	                <input type="text" name="name" class="form-control" placeholder="{{ __('adminstaticword.City') }}">
                  </div>
            	</div>
