...
class AjaxController extends Controller
{
    public function index($apiName, Request $request) {
            ...
            switch ($apiName) {
                ...
                default :
                    return response()->json([
                        'message' => 'Faild',
                        'data' => '<p style="color:red">the ' . $apiName . ' ' . __('backend.function_not_found'). '</p>'
                    ]);
                    break;
            }
        ...
        return response()->json([
            'message' => 'Faild',
            'data' => '<p style="color:red">'.__('backend.not_allowed').'</p>'
        ]);
        ...
        if($file = $request->file('uploadVideo'))
        {
            ...
            if ($tutor == 0){
                ...
                $newTutor->create([
                    ...
                    'work_center'=>($request->work_center?$request->work_center:'NO'),
                    'center'=>($request->center?$request->work_center:" "),
                    'work_hospital'=>($request->work_hospital?$request->work_hospital:"NO"),
                    'hospital'=>($request->hospital?$request->hospital:" "),
                    'work_sanatorium'=>($request->work_sanatorium?$request->work_sanatorium:"No"),
                    'sanatorium'=>($request->wsanatorium?$request->wsanatorium:" "),
               ...
               // dd($updateArray);
                $tutorUpdate->update($updateArray);
			...
			if($request->tab_index == 2)
			{
				$instructor = Instructor::find($tutor_id);
				$instructor->update(['headline' => $request->headline, 'detail' => $request->detail]);
			}
    ...
    public function confirmLesson(Request $request){
        ...
        if(env('MAIL_USERNAME')!=null) {
            try{
                ...
                /*sending email*/
                $request = $appointment;
                $x = __('backend.review_tutor_message', ['student_fname'=>$request->user->fname,'student_lname'=>$request->user->lname]);
    ...
    public function cancelLesson(Request $request){
        ...
        //            return "Error: You can't cancel this lesson as the different between today and the lesson date is less than 3 days";
			...
			return json_encode(['Error'=> __('backend.cancel_lesson_alert_message', ['number'=>1,'param'=>__('backend.day')])]);
			...
			if($paymentTransaction)
            {
				if($paymentTransaction->payment_method == 'flutterwave')
				{
					...
					if($data->status !='success'){
						return json_encode(['Error'=> __('backend.cannot_refund_this_lesson', ['param'=>$data->data])]);
					}
				...
				elseif($paymentTransaction && $paymentTransaction->payment_method == 'myfatoora')
				{
					...
					if($data->IsSuccess != 1){
						return json_encode(['Error'=> __('backend.cannot_refund_this_lesson', ['param'=>$data->Message])]);
					}
